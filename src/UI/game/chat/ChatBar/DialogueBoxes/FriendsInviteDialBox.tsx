...
export default function FriendsInviteDialBox() {
	...
	useEffect(() => {
		...
		FriendService.getOnlineFriends().then(({ data }: { data: FriendStatus[] }) => {
			setFriends(data);
			setIsLoading(false)
	...
	return (
		<div className="gradient-border-2 p-4 h-[50vh] min-w-[29rem]  overflow-y-scroll rounded-xl flex flex-col gap-1">
			...
			{!isLoading && <div className="flex flex-col">
				...
				{friends.map(data => <FriendInvite friendData={data} />)}
			...
			{
				(!isLoading && friends.length == 0) && <div className="flex flex-col flex-1 justify-center">
