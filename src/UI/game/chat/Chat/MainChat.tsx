...
export default function MainChat({
  ...
  if (loading)
    return (
      <div
        className={` bg-secondary  justify-center  rounded-xl drop-shadow-lg flex flex-col gap-3 p-5 overflow-y-auto ${className}`}
      >
        <span className="loader mx-auto scale-75"></span>
      </div>
    )
  else
    return (
      <div
        ref={chatRef}
        className={` bg-secondary   rounded-xl drop-shadow-lg flex flex-col gap-3 p-5 overflow-y-auto ${className}`}
      >
        {paginating && (
