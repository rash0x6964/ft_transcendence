	...
	return (
		<>
			<ContextMenu MenuRef={menuRef} clicked={clicked} pos={position} >
				<MenuBtn onClick={() => setClicked(false)} title="Send Message" />
				<MenuBtn onClick={() => setClicked(false)} title="Profile" />
				{!isBlocked(selectedData) && <MenuBtn onClick={() => { setClicked(false); handleBlock(selectedData) }} title="Block" />}
				{isBlocked(selectedData) && <MenuBtn onClick={() => { setClicked(false); handleUnblock(selectedData) }} title="unBlock" />}
				{!isMuted(selectedData) && <MenuBtn onClick={() => { setClicked(false); handleMute(selectedData) }} title="Mute" />}
				{isMuted(selectedData) && <MenuBtn onClick={() => { setClicked(false); handleUnMute(selectedData) }} title="unMute" />}
				{<MenuBtn onClick={() => { setClicked(false); handleFriendRemove(selectedData, socket) }} title="Unfriend" />}
			</ContextMenu>
